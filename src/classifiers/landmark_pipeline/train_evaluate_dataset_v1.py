"""
Train and evaluate the MLP V1 classifier from dataset/labeled_features_dataset.csv.

V1 config: 3-layer [128,64,32], relu, Adam, LR=1e-3, alpha=1e-4, early_stopping.
...
4. MLPClassifier V1 training          (random_state=42)
5. Evaluation — confusion matrix, precision-recall, confidence curves
6. Save model to models/MLP_V1_dataset/
...
MODELS_DIR = ROOT / 'models' / 'MLP_V1_dataset'
...
def plot_loss_curve(mlp):
    ...
    plt.title('MLP V1 Training Loss Curve')
...
def plot_confidence_curves(thresholds, conf_data):
    ...
    plt.suptitle('MLP V1 Confidence Threshold Curves', fontsize=13, fontweight='bold')
    plt.tight_layout()
...
def save_training_stats(mlp, X, X_train, X_test, y_test, y_pred, report):
    ...
    with open(path, 'w') as f:
        f.write('MLP V1 Training Statistics\n')
        f.write('=' * 50 + '\n\n')
...
def save_curve_stats(mlp, thresholds, conf_data, pr_data):
    ...
    col_w = 12
    with open(path, 'w') as f:
        f.write('MLP V1 Curve Statistics\n')
        f.write('=' * 60 + '\n')
...
def main():
    ...
    print(f'\nModel  → models/MLP_V1_dataset/mlp_model.pkl')
    print(f'Scaler → models/MLP_V1_dataset/scaler.pkl')
