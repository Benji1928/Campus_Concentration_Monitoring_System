"""
Train and evaluate the MLP V2 classifier from dataset/labeled_features_dataset.csv.

V2 config: 3-layer [256,128,64], tanh, Adam, LR=5e-4, alpha=5e-4, early_stopping.
...
4. MLPClassifier V2 training          (random_state=42)
5. Evaluation — confusion matrix, precision-recall, confidence curves
6. Save model to models/MLP_V2_dataset/
...
from src.classifiers.landmark_pipeline.mlp_configs import MLPV2Config
...
MODELS_DIR = ROOT / 'models' / 'MLP_V2_dataset'
...
def plot_loss_curve(mlp):
    ...
    plt.title('MLP V2 Training Loss Curve')
...
def plot_confidence_curves(thresholds, conf_data):
    ...
    plt.suptitle('MLP V2 Confidence Threshold Curves', fontsize=13, fontweight='bold')
    plt.tight_layout()
...
def save_training_stats(mlp, X, X_train, X_test, y_test, y_pred, report):
    ...
    with open(path, 'w') as f:
        f.write('MLP V2 Training Statistics\n')
        f.write('=' * 50 + '\n\n')
...
def save_curve_stats(mlp, thresholds, conf_data, pr_data):
    ...
    col_w = 12
    with open(path, 'w') as f:
        f.write('MLP V2 Curve Statistics\n')
        f.write('=' * 60 + '\n')
...
def main():
    ...
    print(f'\nModel  → models/MLP_V2_dataset/mlp_model.pkl')
    print(f'Scaler → models/MLP_V2_dataset/scaler.pkl')
