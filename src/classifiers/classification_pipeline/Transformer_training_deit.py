"""
DeiT-Tiny Training Script.

V2: deit_tiny_patch16_224 backbone, head [128], CosineAnnealingLR,
    LR=3e-4, BS=32, dropout=0.2, weight_decay=1e-4, patience=5,
    optional class-weight loss, pre-split dataset support.

Usage:
    python src/classifiers/classification_pipeline/Transformer_training_deit.py
"""
...
from src.classifiers.classification_pipeline.transformer_configs import DeiTTinyConfig
...
def main():
    ...
    model.head = build_classifier_head(
        num_features, CONFIG.head_hidden_sizes, CONFIG.num_classes,
        CONFIG.dropout, CONFIG.activation,
    )
    model = model.to(DEVICE)
    print(f'  Features: {num_features} -> head {CONFIG.head_hidden_sizes} -> {CONFIG.num_classes} classes')
    ...
    with open(out_dir / 'training_stats.txt', 'w') as f:
        ...
        f.write(f'Head layers:     {CONFIG.head_hidden_sizes}\n')
