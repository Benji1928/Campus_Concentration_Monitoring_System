"""
CNN Training — V1 Configuration.

V1: EfficientNet-B0 backbone, deeper head [256, 128], CosineAnnealingLR,
    LR=3e-4, BS=64, dropout=0.4, weight_decay=1e-4, patience=5.

Usage:
    python src/classifiers/classification_pipeline/CNN_training_v1.py

If running locally, set CNNV1Config.dataset_dir to your dataset path.
Outputs are saved to models/CNN_V1/ (local) or /kaggle/working/CNN_V1/ (Kaggle).
"""
...
from src.classifiers.classification_pipeline.cnn_configs import CNNV1Config
...
def _resolve_paths():
    """Resolve dataset and output dirs, supporting both Kaggle and local runs."""
    ...
    if not ds.exists():
        ...
        if local.exists():
            ...
            raise FileNotFoundError(
                ...
                "Set CNNV1Config.dataset_dir to your local dataset path."
            )
...
def build_head(num_features, hidden_sizes, num_classes, dropout, activation):
    """Builds a classifier head with BatchNorm + Dropout between each hidden layer."""
    layers = []
